...
START_TEST (test_forchess_knight_checks)
{
	...
	fail_unless(!fc_is_king_in_check(&board, FC_FIRST));
	fc_board_set_piece(&board, FC_SECOND, FC_KNIGHT, 2, 1);
	fail_unless(fc_is_king_in_check(&board, FC_FIRST) == FC_CHECK);
	...
	fail_unless(!fc_is_king_in_check(&board, FC_SECOND));
	...
	fail_unless(!fc_is_king_in_check(&board, FC_THIRD));
	fc_board_set_piece(&board, FC_FOURTH, FC_KNIGHT, 5, 6);
	fail_unless(fc_is_king_in_check(&board, FC_THIRD) == FC_CHECK);
	...
	fail_unless(!fc_is_king_in_check(&board, FC_FOURTH));
	fc_board_set_piece(&board, FC_THIRD, FC_KNIGHT, 1, 5);
	fail_unless(fc_is_king_in_check(&board, FC_FOURTH) == FC_CHECK);
...
START_TEST (test_forchess_pawn_checks)
{
	...
	fail_unless(fc_is_king_in_check(&board, FC_FIRST) == 0);
...
START_TEST (test_forchess_checkmate)
{
	...
	fail_unless(fc_is_king_in_check(&board, FC_FIRST) == FC_CHECK);
	fail_unless(fc_is_king_in_check(&board, FC_SECOND) == FC_CHECKMATE);
}
END_TEST
