...
START_TEST (test_forchess_king_moves)
{
	...
	ret = fc_get_king_moves(&board, &moves, FC_FIRST);
	fail_unless(ret == 1 && moves.index == 3);
	fail_unless(moves.moves[0].move == UINT64_C(0x101));
	fail_unless(moves.moves[1].move == UINT64_C(0x201));
	fail_unless(moves.moves[2].move == UINT64_C(0x3));

	fc_mlist_free(&moves);
	ret = fc_mlist_init(&moves, 0);
	ret = fc_get_king_moves(&board, &moves, FC_THIRD);
	fail_unless(ret == 1 && moves.index == 3);
	fail_unless(moves.moves[0].move == UINT64_C(0xc000000000000000));
	fail_unless(moves.moves[1].move == UINT64_C(0x8040000000000000));
	fail_unless(moves.moves[2].move == UINT64_C(0x8080000000000000));

	fc_mlist_free(&moves);
	ret = fc_mlist_init(&moves, 0);
	ret = fc_get_king_moves(&board, &moves, FC_SECOND);
	fail_unless(ret == 1 && moves.index == 7);
	fail_unless(moves.moves[0].move == UINT64_C(0x0408000000));
	fail_unless(moves.moves[1].move == UINT64_C(0x000c000000));
	fail_unless(moves.moves[2].move == UINT64_C(0x0808000000));
	fail_unless(moves.moves[3].move == UINT64_C(0x0008080000));
	fail_unless(moves.moves[4].move == UINT64_C(0x1008000000));
	fail_unless(moves.moves[5].move == UINT64_C(0x0018000000));
	fail_unless(moves.moves[6].move == UINT64_C(0x0008100000));

	fc_mlist_free(&moves);
	ret = fc_mlist_init(&moves, 0);
	ret = fc_get_king_moves(&board, &moves, FC_FOURTH);
	fail_unless(ret == 0);
}
END_TEST
